...
export default async function collectionUpsertSubscriber({
  event: { data },
  container,
  ...
  try {
    ...
    if (res.ok) {
      logger.info(`[revalidate] Revalidated storefront homepage for collection: ${data.id}`)
    } else {
      logger.warn(
        `[revalidate] Revalidate endpoint returned ${res.status} for collection: ${data.id}`,
      )
    }
  } catch (error) {
    const err = error as Error
    logger.error(
      `[revalidate] Failed to revalidate storefront for collection ${data.id}: ${err.message}`,
    )
  }
}
